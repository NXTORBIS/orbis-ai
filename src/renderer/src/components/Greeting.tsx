...
export default function Greeting({ title }: { title: string }): React.JSX.Element {
  ...
  const prefix = title.trim() ? `Hi ${title.trim()},` : 'Hi there,'
  return (
    <h1 className={`greeting${glitching ? ' glitching' : ''}`} data-text={`${prefix} ${typed}`} aria-label={`${prefix} ${PHRASES[0]}`}>
      <span className="greeting-prefix">{prefix}</span> {typed}
